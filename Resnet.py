...
def train_model(model, train_loader, val_loader, device, epochs=5, checkpoint_dir="checkpoints", excel_path="metrics.xlsx"):
...
resnet_model = train_model(get_model("resnet50", 56),train_loader,val_loader,device)
torch.save(resnet_model.state_dict(), "resnet50_plates.pth")

def evaluate_full_test(model, dataloader, class_names, name="Modelo", export_dir="eval_results"):
...
y_true, y_pred = evaluate_full_test(resnet_model, val_loader, val_dataset.labels, name="ResNet50")
