...
model = CustomDeepCNN(num_classes=56).to(device)
...
train_dataset = PlatesDataset(train_dir, transform=train_transform)
val_dataset = PlatesDataset(val_dir, transform=val_transform)
test_dataset = PlatesDataset(test_dir, transform=val_transform)
...
from tqdm import tqdm
def train_model(model, train_loader, val_loader, device, epochs=60, checkpoint_dir="checkpoints_2", excel_path="metrics2.xlsx"):
...
torch.save(model.state_dict(), "model_plates2.pth")
...
y_true, y_pred = evaluate_full_test(model, val_loader, val_dataset.labels, name="Own Model")

def export_50_predictions(dataset, y_true, y_pred, title="", export_dir="predicted_samples2"):
    os.makedirs(export_dir, exist_ok=True)
...
export_50_predictions(val_dataset, y_true, y_pred, title="Own_CNN")
